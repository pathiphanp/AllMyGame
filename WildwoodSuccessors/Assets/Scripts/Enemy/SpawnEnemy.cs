...
public class SpawnEnemy : MonoBehaviour
{
    ...
    public void BossCallSpawnMonster(float delaySpawn, MonsterType enemyNormal)
    {
        bossMode = true;
        // bossCallSpawn = StartCoroutine(CallSpawnMoster(delaySpawn, enemyNormal));
    }
    public void BossStopSpawnMonster()
    {
        bossMode = false;
        StopCoroutine(bossCallSpawn);
    }
    public void StopSpawnMonster()
    {
        ...
        if (boss != null)
        {
            boss.GetComponent<CallBlackBoss>().CallBackBoss();
        }
    }
